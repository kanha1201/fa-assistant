#!/usr/bin/env python3
"""Test backend with additional questions."""
import sys
import os
import json
import sqlite3
    ...
    for model in models_to_try:
        try:
            ...
            try:
                ...
                pass
            return f"API Error {e.code}: {error_msg if 'error_msg' in locals() else e.reason}"
        except Exception as e:
            continue
    
    return f"Error: Could not connect to Gemini API with any model."
...
"""
    ...
    if "volume" in question.lower():
        prompt = f"{question}\n\nUse the financial data provided. If traded volume is available, provide the specific number. Otherwise, mention that this data is not available in the stored information."
    elif "eps" in question.lower():
        prompt = f"{question}\n\nUse the EPS (Earnings Per Share) value from the financial metrics provided."
    elif "book value" in question.lower():
        prompt = f"{question}\n\nUse the book value from the financial metrics provided."
...
def main():
    """Run all questions."""
    print("\n" + "🚀" * 40)
    print("BACKEND TESTING - Additional Questions")
    print("🚀" * 40)
    ...
    # Questions
    questions = [
        "What is the total traded volume of Eternal?",
        "What is the EPS of Eternal?",
        "What is the book value of Eternal Stock?",
    ...
    # Save answers
    output_file = BASE_DIR / "data" / "processed" / "test_additional_answers.json"
